...
public class AddOperation<T> extends Operation<T> {
    protected Tensor<T> a, b;

    public AddOperation(Tensor<T> a, Tensor<T> b) {
        super(a, b);
    ...
    public Tensor<T> _forward() {
        return TensorUtils.elementWise(a, b, Double::sum);
    }
